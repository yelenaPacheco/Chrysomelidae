...
then
   mkdir -p trimmed_reads_Douglas
fi
 
 
 
if [ ! -d fastp_trimming_docs_Douglass ]
then
  mkdir -p fastp_trimming_docs_Douglas
fi
...
--out1 trimmed_reads_Douglas/${base}_P_R1.fastq --out2 trimmed_reads_Douglas/${base}_P_R2.fastq \
--unpaired1 trimmed_reads_Douglas/${base}_U.fastq --unpaired2 trimmed_reads_Douglas/${base}_U.fastq \
--failed_out fastp_trimming_docs_Douglas/${base}_failed.fastq \
-j fastp_trimming_docs_Douglas4/${base}.fastp_report.json \
-h fastp_trimming_docs_Douglas/${base}.fastp_report.html
